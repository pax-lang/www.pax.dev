...
var Pax = (() => {
  ...
  // src/classes/messages/navigation-patch.ts
  var NavigationPatch = class {
    fromPatch(jsonMessage) {
  ...
  var NAVIGATION_PATCH = "Navigation Patch";
  ...
  function setupEventListeners(chassis) {
    ...
    window.addEventListener("keydown", (evt) => {
      ...
      if (dom_node_selected && !(evt.key === "Escape" || evt.key == "Meta" || evt.key == "Control" || evt.key === "+" || evt.key === "-")) {
        return;
      }
      let event = {
        "KeyDown": {
  ...
  function processMessages(messages2, chassis, objectManager2) {
    ...
    messages2?.forEach((unwrapped_msg) => {
      if (unwrapped_msg["ShrinkLayersTo"]) {
        ...
        let patch = objectManager2.getFromPool(NAVIGATION_PATCH);
        patch.fromPatch(msg);
